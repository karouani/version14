...
@Stateless
public class AjouterManagerAction implements ModelDriven<Manager>{
	...
	public String execute(){
		...
		try {
			...
			if("ManagerAgence".equals(manager.getProfile())){
				...
				col=new ManagerAgence(manager.getMatricule(), manager.getNom(), manager.getPrenom(), manager.getAbreviation(), manager.getSexe(), manager.getDateembauche(), manager.getMoisBAP(),partic, manager.getDateparticipeseminaire(), Double.parseDouble(manager.getSalaireactuel()), manager.getPosttravail(),manager.getEmail());
				System.out.println("Collaborateur "+col);
			}else if(manager.getProfile().equals("AmbassadeurRH")){
				col=new AmbassadeurRH(manager.getMatricule(), manager.getNom(), manager.getPrenom(), manager.getAbreviation(), manager.getSexe(), manager.getDateembauche(), manager.getMoisBAP(), partic, manager.getDateparticipeseminaire(), Double.parseDouble(manager.getSalaireactuel()), manager.getPosttravail(),manager.getEmail());
				System.out.println("Collaborateur "+col);
			}else if(manager.getProfile().equals("ResponsableProduction")){
				col=new ResponsableProduction(manager.getMatricule(), manager.getNom(), manager.getPrenom(), manager.getAbreviation(), manager.getSexe(), manager.getDateembauche(), manager.getMoisBAP(),partic, manager.getDateparticipeseminaire(), Double.parseDouble(manager.getSalaireactuel()), manager.getPosttravail(),manager.getEmail());
				System.out.println("Collaborateur "+col);
			}
			remplirTechno();
			remplirDiplome();
